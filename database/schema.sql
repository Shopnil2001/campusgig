-- Database creation (Commented out for cloud/cPanel hosting compatibility where DB is pre-created)
-- CREATE DATABASE IF NOT EXISTS campusgigs CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
-- USE campusgigs;

SET FOREIGN_KEY_CHECKS = 0;
DROP VIEW IF EXISTS top_freelancers;
DROP TRIGGER IF EXISTS bids_after_update;
DROP TRIGGER IF EXISTS gigs_after_update;
DROP PROCEDURE IF EXISTS freelancer_average_rating;
...
DELIMITER $$
CREATE TRIGGER bids_after_update AFTER UPDATE ON bids
FOR EACH ROW
BEGIN
    IF NEW.status = 'Accepted' AND OLD.status <> 'Accepted' THEN
        UPDATE gigs SET status = 'In Progress' WHERE gig_id = NEW.gig_id AND status = 'Open';
    END IF;
END$$

CREATE TRIGGER gigs_after_update AFTER UPDATE ON gigs
FOR EACH ROW
BEGIN
    IF NEW.status = 'Completed' AND OLD.status <> 'Completed' THEN
        INSERT INTO transactions (gig_id, amount, payment_status, completed_at)
        SELECT NEW.gig_id, COALESCE(MAX(b.proposed_price), NEW.budget), 'Pending', NOW()
        FROM bids b WHERE b.gig_id = NEW.gig_id AND b.status = 'Accepted'
        ON DUPLICATE KEY UPDATE completed_at = NOW();
    END IF;
END$$

CREATE PROCEDURE freelancer_average_rating(IN p_user_id INT, OUT p_average DECIMAL(3,2))
BEGIN
    SELECT COALESCE(AVG(rating), 0.00) INTO p_average FROM reviews WHERE reviewee_id = p_user_id;
END$$
DELIMITER ;
