...
try {
    ...
    if ($action === 'my_gigs') {
        requireAuth($userId);
        $stmt = $pdo->prepare('SELECT g.gig_id, g.client_id, uc.name client_name, uc.department, uc.email client_email, g.title, g.description, g.budget, DATE_FORMAT(g.deadline, "%b %d, %Y") deadline, g.status, g.created_at, COUNT(DISTINCT b.bid_id) bid_count, GROUP_CONCAT(DISTINCT s.skill_name ORDER BY s.skill_name SEPARATOR ", ") skills, MAX(CASE WHEN b.status = "Accepted" THEN b.freelancer_id END) accepted_freelancer_id, MAX(CASE WHEN b.status = "Accepted" THEN uf.name END) accepted_freelancer_name, MAX(CASE WHEN b.status = "Accepted" THEN uf.email END) accepted_freelancer_email FROM gigs g JOIN users uc ON uc.user_id = g.client_id LEFT JOIN bids b ON b.gig_id = g.gig_id LEFT JOIN users uf ON uf.user_id = b.freelancer_id LEFT JOIN gig_skill_required gsr ON gsr.gig_id = g.gig_id LEFT JOIN skills s ON s.skill_id = gsr.skill_id WHERE g.client_id = ? GROUP BY g.gig_id, g.client_id, uc.name, uc.department, uc.email, g.title, g.description, g.budget, g.deadline, g.status, g.created_at ORDER BY g.created_at DESC');
        $stmt->execute([$userId]);
